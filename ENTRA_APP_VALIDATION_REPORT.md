...
**Application ID**: ENTRA_CLIENT_ID_PLACEHOLDER  
...
| Setting | Value | Status | Notes |
|---------|-------|--------|-------|
| **Application ID** | `ENTRA_CLIENT_ID_PLACEHOLDER` | ✅ CORRECT | Standard UUID format |
| **App ID URI** | `api://ENTRA_CLIENT_ID_PLACEHOLDER` | ✅ CORRECT | Follows RFC standards |
...
- `ENTRA_CLIENT_ID_PLACEHOLDER` (GUID format)
- `api://ENTRA_CLIENT_ID_PLACEHOLDER` (App ID URI format)
...
**Sequence**:
1. Copilot Studio redirects to: `https://login.microsoftonline.com/{tenant_id}/oauth2/v2.0/authorize?client_id=ENTRA_CLIENT_ID_PLACEHOLDER&...`
...
- `scopes_supported`: `["api://ENTRA_CLIENT_ID_PLACEHOLDER/access_as_user", "openid", "profile", "email", "offline_access"]`
...
- ✅ ENTRA_AUDIENCE set to `api://ENTRA_CLIENT_ID_PLACEHOLDER`
...
4. **☐ Create Copilot Studio Connector**
   ...
   - Provide OIDC endpoint: `https://YOUR_FUNCTION_APP_NAME.azurewebsites.net/.well-known/openid-configuration`
...
```bash
curl -s https://YOUR_FUNCTION_APP_NAME.azurewebsites.net/.well-known/openid-configuration | jq '.scopes_supported'
```

**Expected Output**:
```json
[
  "api://ENTRA_CLIENT_ID_PLACEHOLDER/access_as_user",
  "openid",
  "profile",
  "email",
  "offline_access"
...
### Check DCR
```bash
curl -X POST https://YOUR_FUNCTION_APP_NAME.azurewebsites.net/oauth/register \
...
| "Invalid audience" error | Token for different app registration | Verify token scope matches `api://ENTRA_CLIENT_ID_PLACEHOLDER` |
...
**Report URL**: [Entra App Registration](https://portal.azure.com/#view/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/~/Overview/appId/ENTRA_CLIENT_ID_PLACEHOLDER)  
**MCP Server URL**: `https://YOUR_FUNCTION_APP_NAME.azurewebsites.net`
