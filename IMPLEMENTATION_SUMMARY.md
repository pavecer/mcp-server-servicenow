...
### ✅ Code Deployed
- Deployed to Azure Function App: `YOUR_FUNCTION_APP_NAME.azurewebsites.net`
- Region: West Europe
- Status: Running
...
ENTRA_AUDIENCE=api://ENTRA_CLIENT_ID_PLACEHOLDER
...
| Application ID URI | ✅ Configured | `api://ENTRA_CLIENT_ID_PLACEHOLDER` |
...
- Token must contain one of: `ENTRA_CLIENT_ID_PLACEHOLDER` or `api://ENTRA_CLIENT_ID_PLACEHOLDER`
...
```bash
curl -s https://YOUR_FUNCTION_APP_NAME.azurewebsites.net/.well-known/openid-configuration | jq '.issuer, .scopes_supported'
```

**Expected Output**:
```json
"https://login.microsoftonline.com/ENTRA_TENANT_ID_PLACEHOLDER/v2.0"
[
...
| "Invalid audience" | Token for different app ID | Verify token scope matches `api://ENTRA_CLIENT_ID_PLACEHOLDER` |
